...
If $A$ is a set, we define the \emph{$\const$-$\var$-extension} 
...
We will treat $0_{\const}$ like an ordinary zero, i. e. if a number in a matrix is omitted, it is $0_{\const}$ and a diagonal matrix only has $0_{\const}$ entries that are not on their diagonal.\\
If $a\in A\times\{\const\}$, then $a$ is \emph{constant} and we write $\St(a)=\const$ (as an abbreviation of \emph{status}. If $a\in\left(A\times\{\var\}\right)\cup\{\var\}$, then $a$ is \emph{variable} and we write $\St(a)=\var$. Finally, we denote the value of $a$ by $\Val(a_{\const})=\Val(a_{\var})=a$ where $\Val(\var)$ is not defined.
\end{Def}
...
c&\text{ if }\left(\underset{i\in I}{\exists}a_i=\var\right)\vee f\left(\left(\Val(a_i)\right)_{i\in I:a_i\ne\var},\cdot\right)\equiv c,\\
\var&\text{ else },
\end{cases}\\
...
The active extension ensures that no constant value will be changed. This will, for instance, be useful in the training function, where we do not want $0_{\const}$ to change as this value encodes that there is no connection between two PUs, as we discussed in \thref{theory-structure:intro}.
\end{Par}
...
is a special case of \thref{cor:standardized-layerization-feedforward}.\\
...
\begin{Bew}
The function from ordinal to set layerization has already been given by \thref{def:layerization}. On the other hand, if there is a set layerization, the order
\[
...
\begin{Par}
By \thref{pro:layerization-11}, we will use the relational and set form of layerization interchangeably from now on. The next definition provides an important stepping stone towards the function of layerizations by allowing us to differentiate a connection $v\conn w$ not only into direction of information but also the global network direction: the flow\footnote{
...
\]
encoding whether the object being thrown is a ball. As the information that an object is being thrown is not a part of the network, the value of that PU is not dependent on any other PU. We thus have an integrative function representing the \emph{a priori} distribution of \text{ball}:
\[
...
\begin{Par}[Input and Output Values]
The input value of a PU $v$ $s^{v:i}$ refers to the value that is fed into a particular node whereas the output value of a PU refers to the value it distributes to its succeeding nodes. Details can be found below in \thref{def:balancedness}.
\end{Par}

\begin{Par}[Extended States]
...
\begin{Lem}[Gradient Descent in a Normal Generative Model]\label{lem:grad-desc-normal}
...
\begin{Cor}[Gradient Descent in an Additive Normal Generative Model]\label{grad-desc-add}
...
%\begin{The}[Backpropagation]\label{the:backpropagation}
%Consider an additive feed-forward network $\mathcal{N}=(\mathcal{V},\theta)$ with the functional structure $f$ and the loss function
%\begin{equation}
%L:\mathbb{R}^{V^{(0)}\cup V^{(L)}}\times\mathbb{R}_{\const,\var}^{\mathcal{V}\times\mathcal{V}}\to\mathbb{R}
%\end{equation}
%The gradient can then be calculated recursively by
%\begin{equation}
%\begin{aligned}
%\tilde{\delta}&:=\frac{\partial L_{\theta}(s^{V^{(0)}},s^{V^{(L)}})}{\partial N_{\theta}(s^{V^{(L)}})}\\
%\delta&:=\frac{\partial f\left(N_{\theta}(s)\right)}{\partial N_{\theta}(s)}\theta^L\In(\tilde{\delta})\\
%\frac{\partial L}{\partial\theta}&:=\delta f\left(N_{\theta}(s)\right)^T
%\end{aligned}
%\end{equation}
%\end{The}
%
%\begin{Bew}
%We observe that the definition of $\delta$, as $\theta$ is an upper triagonal matrix corresponds to the recursive definition
%\begin{align*}
%\hat{\delta}^{V^{(L)}}&:=\tilde{\delta}=\frac{\partial L}{\partial N^{V^{(L)}}}\\
%\underset{0\le l<L}{\forall}\delta^{V^{(l)}}&:=
%\end{align*}
%\end{Bew}
...
\end{align*}
for some $i\in I$. As $\Sigma_{\lambda'}$ is invertible, \thref{jacobis-formula} implies
\begin{align*}
...
\begin{Par}[Biological Plausibility II: Backpropagation and Predictive Coding]
...
The learning rule on $\xi$, as given by \thref{learning-xi}, is clearly Hebbian. If we apply the learning rule to $\theta$ it is still local:
\[
...
\]
Analogously to \thref{lem:predicode-predistate} we can conclude (\ref{eq:convadd}). It is straight-forward to prove by induction that
\[
